...
- `src/utf8.zig` now also exposes consumer-facing `utf8.CellBuffer`
  word-cursor and bounded-range helpers on top of `utf8.CellBufferReader` so
  prompt search/edit callers can keep boundary discovery on the same lower
  display-cell surface instead of rebuilding it locally
...
- `src/status-prompt.zig` now also consumes the shared `utf8.CellBuffer`
  word-cursor and bounded-range helpers for delete-word and completion
  boundaries instead of keeping those search/edit seams partially prompt-local
...
the first shared message-log/write-side producer gap, and one smaller
prompt-side search/edit boundary gap, but it is still not a reopen gate for
`status.c`: the new hit-test consumer still stops at cached status rows
because tmux-style mouse/runtime plumbing is not ported yet, the shared
`server-print`/message-log seam is still reduced relative to tmux's full
`server_client_print` + `file.c` + `window-copy` stack, and the rest of the
multiline/overlay/status runtime remains reduced.
