...
- `src/grid.zig` can now store truthful `GridCell` payloads and now also owns
  reduced `string_cells` plus `grid_reader` seams that capture-style and
  reader-style consumers can reuse, but broader reader/search/editor
  consumers still have not all adopted that shared cell model.
...
| trim, pad, and search by display cells | `Y` | `Y` | `-` | `Y` | `B` | `Y` | open: string trim/pad is shared, `grid.zig` now exposes a reduced shared reader/search helper surface, and the reduced status/prompt renderer now consumes those width rules, but broader search/read/edit consumers are still missing |
