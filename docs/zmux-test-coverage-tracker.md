...
| Format | `src/format-draw.zig` | width trimming, list clipping, style-aware rendering | formatter edge cases | `tmux-museum/src/format-draw.c` | existing inline + `src/format-draw-test.zig` | `test` | high | covered |  | wide-character trimming and rendering now match tmux width semantics |
