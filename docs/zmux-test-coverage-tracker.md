...
| Format | `src/cmd-list-panes.zig` | list-panes / list-clients shared formatter path | user-visible semantics; ordering | `tmux-museum/src/cmd-list-panes.c` | existing inline + session lifecycle tests | `test` | high | scanned |  | command consumer of format |
| Format | `src/cmd-display-message.zig` | message rendering through shared formatter | protocol-visible semantics; output shape | `tmux-museum/src/cmd-display-message.c` | existing inline | `test` | medium | unscanned |  | likely follow-on facet |
...
| Input/TTY | `src/input.zig` | partial sequences and mode transitions | streamed triggers; parser edge cases | `tmux-museum/src/input.c` | existing inline | `test` | high | scanned |  |  |
...
| Input/TTY | `src/tty-keys.zig` | key decode/encode compatibility | protocol-visible semantics | `tmux-museum/src/tty-keys.c` | existing inline | `test` | medium | scanned |  |  |
...
| Runtime | `src/session.zig` | session lifecycle and side effects | transactional behavior | `tmux-museum/src/session.c` | mixed | `test` | medium | unscanned |  |  |
| Runtime | `src/window.zig` | window lifecycle and state propagation | transactional behavior; side effects | `tmux-museum/src/window.c` | mixed | `test` | high | unscanned |  |  |
...
| Command sweep | `src/cmd-*.zig` | scary semantic buckets | varies | matching tmux command file | mixed | `test` / `test-stress` | medium | unscanned |  | prioritize by scary semantics, not filename order |
...
- Point every intentional red test at `docs/zmux-todo-test-fails.md`.
