...
| `server-client-test.test.server_client_dispatch_for_test ignores command messages once CLIENT_EXIT is set` | `zig-unit` | `red` | tmux drops `MSG_COMMAND` once shutdown has started, but zmux still queues the default command because `server_client_dispatch_command()` does not guard `CLIENT_EXIT`. | Return early for command dispatch when `CLIENT_EXIT` is already set, matching tmux `server_client_dispatch_command()`. |
