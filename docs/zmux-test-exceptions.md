...
| Broad `zig build test -- --test-filter "window"` reruns still pick up 3 pre-existing non-window-suite failures (`tty_draw_render_window_region clears clipped leading wide-cell padding`, `cmd-set-option set-window-option exact matches win over longer prefixed names`, `cmd-respawn-window restarts the only pane in a window`) | `zig-unit` | `red` | The harness filter matches unrelated suites whose names contain `window`; the new `window-test.zig` coverage passes, but these older failures still keep the broad selector red. | Fix the underlying tty-draw, cmd-set-option, and cmd-respawn-window bugs or tighten the campaign rerun selector once those modules have dedicated green lanes. |
