...
| `smoke-recursive:tmux-in-tmux` | `smoke-recursive` | `skip-global` | The pure tmux recursive attach timeout assumption is not a stable oracle baseline today. | Rework the recursive harness to measure a stable tmux baseline, then restore. |
