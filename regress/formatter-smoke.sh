...
CLIENT_OUT="$TEST_TMPDIR/list-clients-control.out"
CLIENT_ERR="$TEST_TMPDIR/list-clients-control.err"
(
	printf 'refresh-client -C 90,30\n'
	sleep 5
	printf 'detach-client\n'
) | smoke_bin -f/dev/null -C attach-session -t fmtcheck >"$CLIENT_OUT" 2>"$CLIENT_ERR" &
CLIENT_PID=$!
