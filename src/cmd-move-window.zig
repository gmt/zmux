...
fn exec(cmd: *cmd_mod.Cmd, item: *cmdq.CmdqItem) T.CmdRetval {
    ...
    if (server_fn.server_link_window(src_s, src_wl, dst_s, dst_idx, args.has('k'), !args.has('d'), &cause) != 0) {
    ...
    if (cmd.entry == &entry_move) {
        ...
        if (!args.has('s') and opts.options_get_number(src_s.options, "renumber-windows") != 0)
            sess.session_renumber_windows(src_s);
    }
    return .normal;
}
...
pub const entry_move: cmd_mod.CmdEntry = .{
    .name = "move-window",
    .alias = "movew",
    .usage = "[-abdkr] [-s source-window] [-t target-window]",
    .template = "abdkrs:t:",
    .lower = 0,
    .upper = 0,
    .flags = 0,
...
pub const entry_link: cmd_mod.CmdEntry = .{
    .name = "link-window",
    .alias = "linkw",
    .usage = "[-abdk] [-s source-window] [-t target-window]",
    .template = "abdks:t:",
    .lower = 0,
    .upper = 0,
    .flags = 0,
...
test "link-window -a and move-window -b shuffle around the target window" {
    const env_mod = @import("environ.zig");
    const spawn = @import("spawn.zig");
    const win = @import("window.zig");
    ...
    const src = sess.session_create(null, "shuffle-src", "/", env_mod.environ_create(), opts.options_create(opts.global_s_options), null);
    defer if (sess.session_find("shuffle-src") != null) sess.session_destroy(src, false, "test");
    const dst = sess.session_create(null, "shuffle-dst", "/", env_mod.environ_create(), opts.options_create(opts.global_s_options), null);
    defer if (sess.session_find("shuffle-dst") != null) sess.session_destroy(dst, false, "test");

    var cause: ?[]u8 = null;
    var src_a: T.SpawnContext = .{ .s = src, .idx = -1, .flags = T.SPAWN_EMPTY };
    const src_wl0 = spawn.spawn_window(&src_a, &cause).?;
    var src_b: T.SpawnContext = .{ .s = src, .idx = -1, .flags = T.SPAWN_EMPTY };
    const src_wl1 = spawn.spawn_window(&src_b, &cause).?;
    var dst_a: T.SpawnContext = .{ .s = dst, .idx = -1, .flags = T.SPAWN_EMPTY };
    const dst_wl0 = spawn.spawn_window(&dst_a, &cause).?;
    var dst_b: T.SpawnContext = .{ .s = dst, .idx = -1, .flags = T.SPAWN_EMPTY };
    const dst_wl1 = spawn.spawn_window(&dst_b, &cause).?;

    const src_window0 = src_wl0.window;
    const src_window1 = src_wl1.window;
    const dst_window0 = dst_wl0.window;
    const dst_window1 = dst_wl1.window;

    var parse_cause: ?[]u8 = null;
    const link_cmd = try cmd_mod.cmd_parse_one(&.{ "link-window", "-a", "-s", "shuffle-src:0", "-t", "shuffle-dst:0" }, null, &parse_cause);
    defer cmd_mod.cmd_free(link_cmd);

    var list: cmd_mod.CmdList = .{};
    var item = cmdq.CmdqItem{ .client = null, .cmdlist = &list };
    try std.testing.expectEqual(T.CmdRetval.normal, cmd_mod.cmd_execute(link_cmd, &item));
    try std.testing.expectEqual(dst_window0, sess.winlink_find_by_index(&dst.windows, 0).?.window);
    try std.testing.expectEqual(src_window0, sess.winlink_find_by_index(&dst.windows, 1).?.window);
    try std.testing.expectEqual(dst_window1, sess.winlink_find_by_index(&dst.windows, 2).?.window);

    const move_cmd = try cmd_mod.cmd_parse_one(&.{ "move-window", "-b", "-s", "shuffle-src:1", "-t", "shuffle-dst:2" }, null, &parse_cause);
    defer cmd_mod.cmd_free(move_cmd);
    try std.testing.expectEqual(T.CmdRetval.normal, cmd_mod.cmd_execute(move_cmd, &item));
    try std.testing.expect(sess.winlink_find_by_index(&src.windows, 1) == null);
    try std.testing.expectEqual(dst_window0, sess.winlink_find_by_index(&dst.windows, 0).?.window);
    try std.testing.expectEqual(src_window0, sess.winlink_find_by_index(&dst.windows, 1).?.window);
    try std.testing.expectEqual(src_window1, sess.winlink_find_by_index(&dst.windows, 2).?.window);
    try std.testing.expectEqual(dst_window1, sess.winlink_find_by_index(&dst.windows, 3).?.window);
}

test "move-window without -s renumbers the source session when configured" {
    const env_mod = @import("environ.zig");
    const spawn = @import("spawn.zig");
    const win = @import("window.zig");
    ...
    const src = sess.session_create(null, "renumber-src", "/", env_mod.environ_create(), opts.options_create(opts.global_s_options), null);
    defer if (sess.session_find("renumber-src") != null) sess.session_destroy(src, false, "test");
    const dst = sess.session_create(null, "renumber-dst", "/", env_mod.environ_create(), opts.options_create(opts.global_s_options), null);
    defer if (sess.session_find("renumber-dst") != null) sess.session_destroy(dst, false, "test");

    opts.options_set_number(src.options, "renumber-windows", 1);
    ...
    var src_b: T.SpawnContext = .{ .s = src, .idx = -1, .flags = T.SPAWN_EMPTY };
    const src_wl1 = spawn.spawn_window(&src_b, &cause).?;
    var src_c: T.SpawnContext = .{ .s = src, .idx = -1, .flags = T.SPAWN_EMPTY };
    const src_wl2 = spawn.spawn_window(&src_c, &cause).?;
    var dst_a: T.SpawnContext = .{ .s = dst, .idx = -1, .flags = T.SPAWN_EMPTY };
    _ = spawn.spawn_window(&dst_a, &cause).?;

    var parse_cause: ?[]u8 = null;
    const move_cmd = try cmd_mod.cmd_parse_one(&.{ "move-window", "-t", "renumber-dst:3" }, null, &parse_cause);
    defer cmd_mod.cmd_free(move_cmd);
    ...
    var state = cmdq.CmdqState{
        .current = .{
            ...
            .idx = src_wl1.idx,
            .w = src_wl1.window,
            .wp = src_wl1.window.active,
        },
    };
    var item = cmdq.CmdqItem{ .client = null, .cmdlist = &list, .state = &state };
    try std.testing.expectEqual(T.CmdRetval.normal, cmd_mod.cmd_execute(move_cmd, &item));

    try std.testing.expectEqual(src_wl2.window, sess.winlink_find_by_index(&src.windows, 1).?.window);
