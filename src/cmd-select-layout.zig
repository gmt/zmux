...
fn exec(cmd: *cmd_mod.Cmd, item: *cmdq.CmdqItem) T.CmdRetval {
    ...
    w.old_layout = layout_mod.dump_window(w);
...
test "select-layout undo and explicit layout strings restore dumped geometry" {
    ...
    const dumped = layout_mod.dump_window(wl.window).?;
    ...
    try std.testing.expectEqual(@as(u32, 60), first.sx);
    try std.testing.expectEqual(@as(u32, 61), second.xoff);
    try std.testing.expectEqual(@as(u32, 19), second.sx);
