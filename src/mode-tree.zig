...
/// Default context-menu items shown when right-clicking outside a tree
/// item (tmux `mode_tree_menu_items`).
pub const mode_tree_menu_items = [_]menu_mod.MenuItemTemplate{
...
pub const DrawCallback = *const fn (?*anyopaque, ?*anyopaque, *T.ScreenWriteCtx, u32, u32) void;
...
pub const Config = struct {
    ...
    zoom: bool = false,
    menu: []const menu_mod.MenuItemTemplate = &.{},
...
pub fn toggleCurrentTag(mtd: *Data, advance: bool) void {
    ...
    if (!current_item.tagged) {
        ...
        mode_tree_clear_tagged(&current_item.children);
...
pub fn draw(mtd: *Data) void {
    ...
    if (box_x != 0 and box_y != 0) {
        ...
        if (mtd.drawcb) |drawcb|
            drawcb(mtd.modedata, draw_mti.itemdata, &ctx, box_x, box_y);
    }
...
/// tmux `mode_tree_run_command(c, fs, template, name)`.
/// Expand `template` with `name` replacing `%%`, parse the result, and
/// append it to the client's command queue.  `fs` may be null, in which
/// case the command is queued with a blank find-state (matching tmux
/// behaviour when callers pass `NULL`).
pub fn runCommand(client: ?*T.Client, fs: ?*const T.CmdFindState, template: []const u8, name: []const u8) void {
    ...
    const expanded = templateReplace(template, name, 1);
...
/// tmux `mode_tree_display_menu` – build and show a context menu.
/// When `outside` is true the generic tree menu items are used; otherwise
/// the mode-specific menu stored on `mtd` is shown.
pub fn displayMenu(mtd: *Data, client: ?*T.Client, x: u32, y: u32, outside: bool) void {
    ...
    const line: u32 = if (mtd.offset + y > mtd.line_list.items.len -| 1)
        ...
        mtd.offset + y;
    if (line >= mtd.line_list.items.len) return;
    const mti = mtd.line_list.items[line].item;

    const items = if (outside) &mode_tree_menu_items else mtd.menu;
...
/// Replace the first `%%` or `%idx` in `template` by `replacement`
/// (tmux `cmd_template_replace`).
